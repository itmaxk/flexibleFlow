...
def build_mikrotik_bootstrap_fallback(foreign):
    return (
        ...
        f":local VLESSSERVER \"{foreign['address']}\"\n"
        f":local VLESSPORT {foreign['port']}\n"
...
def render_mikrotik_bootstrap_content(foreign):
    ...
    content = re.sub(r':local VLESSSERVER "[^"]*"', f':local VLESSSERVER "{foreign["address"]}"', content, count=1)
    content = re.sub(r":local VLESSPORT \d+", f':local VLESSPORT {foreign["port"]}', content, count=1)
